...
@else
    @php $hasOutOfStock = collect($lines)->contains(fn ($l) => $l['product']->track_stock && $l['variant']->isOutOfStock()); @endphp
            ...
            @foreach ($lines as $line)
                <div class="p-4 flex items-center gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        @if (! empty($line['label']))<div class="text-xs text-slate-500">{{ $line['label'] }}</div>@endif
                        <div class="text-sm text-slate-400">{{ $symbol }} {{ number_format($line['unit_price'], 2) }} each</div>
                        @if ($line['variant']->isOutOfStock() && $line['product']->track_stock)
