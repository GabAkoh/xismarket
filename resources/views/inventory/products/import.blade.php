    ...
    <x-card>
        ...
        <div x-show="source === 'odoo'" x-cloak class="mt-4 rounded-md bg-slate-50 border border-slate-100 p-3 text-xs text-slate-500 space-y-1">
            ...
            <div x-show="mode === 'images'" x-cloak class="space-y-1">
                <p><span class="font-medium text-slate-600">Images:</span> fills the main image of products already here that <span class="font-medium">don't have one yet</span> (matched by Internal Reference, then name) — existing images are left untouched. Export with the <span class="font-medium">Image</span> field included (uncheck "I want to update data" in Odoo's export dialog to expose it) — Odoo embeds it as base64 in the CSV, because the public <code>/web/image</code> URL only returns a placeholder for unpublished products.</p>
